...
<style>
    ...
    [data-theme="light"] .order-items {
        background: rgba(166, 131, 42, 0.05); 
    }
...
<div class="page-container">
    ...
    <div id="orderList">
        {{-- 🔄 LOOPING TRANSAKSI DINAMIS DARI DATABASE --}}
        @forelse($orders as $order)
            @php
                // 1. Ambil status asli dari DB dan seragamkan menjadi huruf kecil
                $statusRaw = strtolower($order->StatusOrder);
                
                // 2. Pemetaan status DB (Termasuk settlement & capture dari Midtrans)
                $filterStatus = 'pending';
                
                if (in_array($statusRaw, ['paid', 'selesai', 'success', 'settlement', 'capture'])) {
                    $filterStatus = 'selesai';
                } elseif (in_array($statusRaw, ['proses', 'diproses'])) {
                    $filterStatus = 'proses';
                } elseif (in_array($statusRaw, ['batal', 'cancel', 'dibatalkan'])) {
                    $filterStatus = 'batal';
                }
            @endphp
            ...
            <div class="order-card" data-status="{{ $filterStatus }}">
                <div class="order-top">
                    <div>
                        <div class="order-id">#{{ $order->order_code ?? $order->OrderID }}</div>
                        <div class="order-date">
                            {{ $order->TanggalOrder ? \Carbon\Carbon::parse($order->TanggalOrder)->translatedFormat('d F Y • H:i') : '-' }}
                        </div>
                    </div>
                ...
                <div class="order-items">
                    @if(isset($order->details_summary))
                        {{ $order->details_summary }}
                    @else
                        Pesanan oleh: {{ $order->pelanggan ?? 'Pelanggan NONGKI' }}
                    @endif
                </div>

                <div class="order-bottom">
                    @if($filterStatus == 'selesai')
                        {{-- 🔥 SEKARANG TOMBOL PESAN LAGI TERINTEGRASI DENGAN ROUTE CHECKOUT INSTAN KAMU --}}
                        <form action="{{ route('checkout.instan', ['id' => $order->OrderID]) }}" method="POST" style="display: inline;">
                            @csrf
                    ...
                    @else
                        <a href="{{ route('order.success', ['order_id' => $order->order_code ?? $order->OrderID]) }}" class="reorder-btn" style="background: transparent; color: var(--gold); display: inline-flex; align-items: center; gap: 6px; text-decoration: none; cursor: pointer;">
                    ...
                    <div class="order-total-group">
                        ...
                        <span class="order-total">Rp {{ number_format($order->TotalHarga ?? $order->totalharga, 0, ',', '.') }}</span>
                    </div>
                </div>
            </div>
        @empty
            <div style="text-align: center; padding: 4rem 1rem; color: var(--text-muted); font-family: 'DM Sans', sans-serif;">
